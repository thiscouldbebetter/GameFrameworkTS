...
class Mesh {
    ...
    static fromFace(center, faceToExtrude, thickness) {
        ...
        var faceBuilders = [];
        for (var z = 0; z < 2; z++) {
            var offsetForExtrusion = new Coords(0, 0, (z == 0 ? -1 : 1)).multiplyScalar(thicknessHalf);
            var vertexIndicesTopOrBottom = [];
            for (var v = 0; v < numberOfFaceVertices; v++) {
                var vertexIndex = meshVertices.length;
                if (z == 0) {
                    var vertexIndexNext = NumberHelper.wrapToRangeMinMax(vertexIndex + 1, 0, numberOfFaceVertices);
