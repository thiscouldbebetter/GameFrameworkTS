...
class EnvironmentMock
{
	universeCreate
	(
		universeCreated: (u: Universe) => void
	): void
	{
		...
		(
			...
			(mediaFilePaths: string[] ) =>
				this.universeBuild_MediaFilePathsLoaded(mediaFilePaths, universeCreated)
		);
	}
	...
	(
		mediaFilePaths: string[],
		universeInitialized: (u: Universe) => void
	): void
	{
		...
		(
			(u: Universe, wc: WorldCreator) => new WorldDemo
			(
				WorldDemo.name,
				DateTime.now(),
				null, // defn
