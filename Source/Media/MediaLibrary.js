...
(function (ThisCouldBeBetter) {
    ...
    (function (GameFramework) {
        class MediaLibrary {
            ...
            static mediaFilePathsReadFromContentDirectoryPathAndManifestFileNameThen(contentDirectoryPath, manifestFileName, callback) {
                var manifestFilePath = contentDirectoryPath + manifestFileName;
                var manifestFileAsTextString = GameFramework.TextString.fromNameAndSourcePath("Manifest", manifestFilePath);
                manifestFileAsTextString.loadThen((textStringLoadedAsLoadable) => {
                    var textStringLoaded = textStringLoadedAsLoadable;
                    var mediaFilePathsRelative = [];
                    var mediaFilePathsAsString = textStringLoaded.value;
                    // Ignore anything in the root content directory.
                    mediaFilePathsAsString =
                        mediaFilePathsAsString.substr(mediaFilePathsAsString.indexOf("+"));
                    var newline = "\n";
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (lineContainsDot) {
                            var filePath = ancestorDirectoryNames.join("/") + "/" + line;
