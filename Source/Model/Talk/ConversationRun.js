...
class ConversationRun {
    constructor(defn, quit, entityPlayer, entityTalker) {
        ...
        this.scopeCurrent = new ConversationScope(null, // parent
        talkNodeStart, 
        // talkNodesForOptions
        []);
    ...
    toControl(size, universe) {
        ...
        var listSize = new Coords(size.x - marginSize.x * 3 - buttonSize.x, size.y - portraitSize.y - marginSize.y * 4, 0);
        var next = () => {
        ...
        var returnValue = new ControlContainer("containerConversation", new Coords(0, 0, 0), // pos
        size, 
        // children
        [
            ...
            new DataBinding(conversationRun, (c) => { return c.scopeCurrent.displayTextCurrent; }, null), fontHeight),
            new ControlLabel("labelResponse", new Coords(marginSize.x, marginSize.y * 2 + portraitSize.y - fontHeight / 2, 0), size, // size
            false, // isTextCentered
            "Response:", fontHeight),
            new ControlList("listResponses", new Coords(marginSize.x, marginSize.y * 3 + portraitSize.y, 0), listSize, 
            // items
            new DataBinding(conversationRun, (c) => { return c.scopeCurrent.talkNodesForOptionsActive(); }, null), 
            // bindingForItemText
            ...
            viewLog, // click
            null, null),
            new ControlButton("buttonDone", new Coords(size.x - marginSize.x - buttonSize.x, size.y - marginSize.y - buttonSize.y, 0), buttonSize.clone(), "Done", fontHeight, true, // hasBorder
            true, // isEnabled
            back, // click
            null, null),
        ], // children
    ...
    toControlTranscript(size, universe, venueToReturnTo) {
        ...
        var buttonHeight = 25;
        var marginSize = new Coords(1, 1, 0).multiplyScalar(marginWidth);
        var listSize = new Coords(size.x * .75, size.y - labelHeight - marginSize.y * 3, 0);
        var returnValue = new ControlContainer("containerConversation", new Coords(0, 0, 0), // pos
        size, 
        // children
        [
            new ControlButton("buttonBack", marginSize, // pos
            ...
            new ControlLabel("labelTranscript", new Coords(size.x / 2, marginSize.y, 0), // pos
            size, // size
            true, // isTextCentered
            "Transcript", fontHeight),
            new ControlList("listEntries", new Coords((size.x - listSize.x) / 2, marginSize.y * 2 + labelHeight, 0), listSize, 
            // items
            new DataBinding(conversationRun, (c) => { return c.talkNodesForTranscript; }, null), new DataBinding(null, (c) => { return c.textForTranscript(conversationDefn); }, null), // bindingForItemText
